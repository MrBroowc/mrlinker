...
import NavbarAdmin from "broowc/components/Admin/NavbarAdmin";
import { useRouter } from "next/router";
import { useAuth } from "broowc/context/AuthContext";
import { Collapse, Form, Input, message } from "antd";
import PreviewWeb from "broowc/components/Admin/PreviewWeb";
import {
  GetSubCollection,
  addDataDoc,
  getUser,
  updateDataDoc,
} from "broowc/utils/helpers";
import { ArrowDownOutlined, PicCenterOutlined } from "@ant-design/icons";
import { serverTimestamp } from "firebase/firestore";
import LoadingPage from "broowc/components/Admin/LoadingPage";
import CardLink from "broowc/components/Admin/CardLink";
import CardHeader from "broowc/components/Admin/CardHeader";
import { useMediaQuery } from "react-responsive";
import ModalShareButton from "broowc/components/Admin/ModalShareButton";
import { useAppContext } from "broowc/context/AppContext";
import Metadata from "broowc/components/Metadata";
import { DndProvider } from "react-dnd";
import { HTML5Backend } from "react-dnd-html5-backend";
import { TouchBackend } from "react-dnd-touch-backend";
import ModalHistoryVisitors from "broowc/components/Modal/ModalHistoryVisitors";
import { PARAMS } from "broowc/constants/constants";
import ModalHistoryLinkClicks from "broowc/components/Modal/ModalHistoryLinkClicks";
import { getMessaging, getToken, onMessage } from "firebase/messaging";
import { app } from "broowc/config/firebase";
...
export default function LinksPage() {
  ...
  useEffect(() => {
    ...
    let foundUser = localStorage.getItem("user_broowc_linker");
  ...
  const onSubmit = (values) => {
    if (links.length >= 10)
      return message.warning("Link/Header has reached maximum limit");
    let data = {
    ...
    addDataDoc(`users/${user?.id}/links`, data)
      .then(() => {
        form.resetFields();
        message.success("Successfully added link!");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleClickAddHeader = () => {
    ...
    addDataDoc(`users/${user?.id}/links`, data)
      .then(() => {
        message.success("Successfully added header!");
      })
      .catch((err) => {
        console.log(err);
      });
              ...
              images: [
                {
                  ...
                  width: 800,
                  height: 800,
                  alt: "Create your own broowclinker",
                },
              ],
            }}
                              ...
                              <Form.Item
                                ...
                                rules={[
                                  ...
                                  },
                                  {
                                    type: "url",
                                    message:
                                      "e.g: https://broowclink.vercel.app",
                                  },
                                  {
                                    type: "string",
                                    min: 6,
